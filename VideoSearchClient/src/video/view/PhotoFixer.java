...
public class PhotoFixer extends ImageView {
	...
	public PhotoFixer(Context context, Bitmap photo) {
		...
		Path p = new Path();
		paint.setPathEffect(new PathDashPathEffect(p, 3, 2,
	...
	public Bitmap getFixedBitmap() {
		if (startX == 0 && startY == 0 && endX == 0 && endY == 0) {
			...
			setDrawingCacheEnabled(true);
			Bitmap res = Bitmap.createBitmap(getDrawingCache(), x + 2, y + 2,
					width - 4, height - 4);
			setDrawingCacheEnabled(false);
			return res;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(new Rect((int) startX, (int) startY, (int) nowX,
				(int) nowY), paint);
